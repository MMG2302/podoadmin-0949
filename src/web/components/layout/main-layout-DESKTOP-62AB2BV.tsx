...
export const MainLayout = ({ children, title }: MainLayoutProps) => {
  ...
  // Detección de viewport móvil para abrir overlay en lugar de toggle desktop
  useEffect(() => {
  ...
  const handleMenuButtonClick = useCallback(() => {
    ...
    handleToggleDesktop();
  }, [isMobileView, handleToggleDesktop]);
  ...
  return (
    <div className="h-dvh h-screen min-h-0 flex overflow-hidden bg-gray-50 dark:bg-gray-950">
      ...
      {/* Main content area - z-[55] so header/menu stay above sidebar (z-50) on tunnel/first load */}
      <div
        className={`relative z-[55] flex-1 flex flex-col min-w-0 min-h-0 md:pl-0 ml-0 w-full overflow-hidden transition-[margin] duration-300 ${
          sidebarVisibleOnDesktop ? "md:ml-72" : "md:ml-0"
        }`}
      >
        {/* Top header - responsive */}
        <header className="flex-shrink-0 sticky top-0 z-40 bg-white dark:bg-gray-900 border-b border-gray-100 dark:border-gray-800 safe-area-top">
          <div className="flex items-center justify-between px-3 sm:px-4 md:px-6 lg:px-8 h-14 sm:h-16">
            {/* Left side - menu button and title (z-10 so it stays above Dock magnification) */}
            <div className="relative z-10 flex items-center gap-2 sm:gap-3 min-w-0 flex-1">
              ...
              {/* Menú: en móvil abre overlay (z-60), en desktop alterna visible/oculto */}
              <button
                type="button"
                onClick={handleMenuButtonClick}
                className="relative z-10 p-2 -ml-1 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-800 active:bg-gray-200 dark:active:bg-gray-700 transition-colors min-w-[44px] min-h-[44px] flex items-center justify-center flex-shrink-0 touch-manipulation"
                aria-label={isMobileView ? "Abrir menú" : (sidebarVisibleOnDesktop ? "Ocultar menú" : "Mostrar menú")}
              >
        ...
        {/* Page content - min-w-0 so flex child can shrink; overflow for scroll on narrow viewports */}
        <main className="flex-1 min-h-0 min-w-0 overflow-y-auto overflow-x-hidden p-3 sm:p-4 md:p-6 lg:p-8 pb-safe bg-gray-50 dark:bg-gray-950">
          {children}
        </main>
      </div>
    </div>
