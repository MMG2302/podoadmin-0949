...
import { resolveLayoutSectionLabel } from "../i18n/clinical-labels";
...
const SessionsPage = () => {
  ...
  const dateLocale =
    language === "en" ? "en-US" : language === "pt" ? "pt-BR" : language === "fr" ? "fr-FR" : "es-ES";
  const { user } = useAuth();
  ...
  const getPatientName = useCallback(
    (patientId: string) => {
      ...
      return patient ? `${patient.firstName} ${patient.lastName}` : t.sessions.unknownPatient;
    },
    [sessions, getPatientById, t]
  );
  ...
  const applyTemplateById = useCallback(
    (templateId: string): boolean => {
      ...
      if (
        ...
        !window.confirm(t.sessions.confirmApplyTemplate)
      ) {
        return false;
      }
  ...
  useEffect(() => {
    const handleBeforePrint = () => {
      ...
      printWindow.document.write(`
        ...
        <html>
        <head>
          <title>${t.sessions.printTitlePrefix} - ${user?.name || t.sessions.userFallback}</title>
          <style>
        ...
        <body>
          <div class="podiatrist-name">
            ${user?.name || t.sessions.userFallback}
          </div>
        </body>
        </html>
      `);
  ...
  }, [showForm, editingSession, formData.patientId, user?.id, user?.name, t]);
  ...
  const handleImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const toDataUrl = (file: File): Promise<string> =>
      compressImageForSession(file).catch((err) => {
        ...
        return new Promise((resolve, reject) => {
          ...
          reader.onerror = () => reject(new Error(t.sessions.imageReadError));
    ...
    for (const file of slice) {
      if (!file.type.startsWith("image/")) {
        setImageUploadError(t.sessions.imageOnlyAllowed);
        continue;
      }
      try {
        ...
        setImageUploadError(err instanceof Error ? err.message : t.sessions.imageProcessError);
      }
    }
  ...
  setGraceError(null);
    ...
    if (!formData.patientId) {
      alert(t.sessions.selectPatientAlert);
      return;
    }
      ...
      if (editingSession) {
        ...
        if (response.success && response.data?.success) {
          ...
          if (errorCode === "usuario_en_periodo_gracia") {
            setGraceError(
              errData?.message || t.sessions.gracePeriodMessage
            );
            return;
          }
  ...
  const handleDelete = async (session: ClinicalSession) => {
    ...
    try {
      ...
      if (response.success && response.data?.success) {
        ...
        alert(response.error || response.data?.message || t.sessions.deleteFailed);
      }
    } catch (error) {
  ...
  const handleExport = async (session: ClinicalSession) => {
    if (!isPodiatrist) {
      alert(t.sessions.exportOnlyPodiatrists);
      return;
    }
    ...
    if (!res.success || !res.data?.export) {
      alert(res.error || t.sessions.exportFailed);
      return;
    }
  ...
  const handleCreatePrescription = async () => {
    ...
    if (!prescriptionText) {
      setPrescriptionFormError(t.sessions.prescriptionRequireContent);
      return;
    }
    ...
    if (!patient) {
      ...
      setPrescriptionFormError(t.sessions.patientLoadFailed);
      return;
    }
    ...
    const res = await api.post<{
      ...
      "/prescriptions",
      {
        ...
        patientWeightKg,
        patientHeightCm,
        podiatristName: user.name?.trim() || prescriptionFormContext?.podiatristName || t.sessions.professionalFallback,
    ...
    <MainLayout title={t.sessions.title}>
      ...
      {selectedSession && (
        <AppModal open onClose={() => setSelectedSession(null)} maxWidth="3xl">
            ...
            <AppModalBody className="space-y-6">
              ...
              {showClinicalSection("anamnesis") && selectedSession.anamnesis && (
                <div>
                  <h4 className="font-medium text-brand-ink mb-2">
                    {resolveLayoutSectionLabel(t, clinicalLayout, "anamnesis")}
                  </h4>
              ...
              <div>
                <h4 className="font-medium text-brand-ink mb-3">{t.sessions.podiatryExamTitle}</h4>
                <PodiatryExaminationFields
              ...
              {showClinicalSection("physical_examination") && selectedSession.physicalExamination && (
                <div>
                  <h4 className="font-medium text-brand-ink mb-2">
                    {resolveLayoutSectionLabel(t, clinicalLayout, "physical_examination")}
                  </h4>
              ...
              {showClinicalSection("diagnosis") && selectedSession.diagnosis && (
                <div>
                  <h4 className="font-medium text-brand-ink mb-2">
                    {resolveLayoutSectionLabel(t, clinicalLayout, "diagnosis")}
                  </h4>
              ...
              {showClinicalSection("treatment_plan") && selectedSession.treatmentPlan && (
                <div>
                  <h4 className="font-medium text-brand-ink mb-2">
                    {resolveLayoutSectionLabel(t, clinicalLayout, "treatment_plan")}
                  </h4>
              ...
              {showClinicalSection("clinical_notes") && selectedSession.clinicalNotes && (
                <div>
                  <h4 className="font-medium text-brand-ink mb-2">
                    {resolveLayoutSectionLabel(t, clinicalLayout, "clinical_notes")}
                  </h4>
                  ...
                  <div className="flex flex-wrap gap-4">
                    {selectedSession.images.map((img, idx) => (
                      <img
                        key={idx}
                        src={img}
                        alt={t.sessions.imageAlt.replace("{n}", String(idx + 1))}
              ...
              {canCreatePrescriptions(user?.role) && (
                <div className="pt-4 border-t border-gray-100">
                  <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between mb-3">
                    <h4 className="font-medium text-brand-ink">{t.sessions.prescriptions.sectionTitle}</h4>
                    <button
                      onClick={() => {
                        setPrescriptionData(EMPTY_PRESCRIPTION_FORM);
                        setPrescriptionFormContext(null);
                    ...
                    >
                      ...
                      {t.sessions.prescriptions.newPrescription}
                    </button>
                  </div>
                  ...
                  {sessionPrescriptions.length > 0 ? (
                      ...
                      {sessionPrescriptions.map((rx) => (
                        ...
                        >
                          <div className="flex-1 min-w-0">
                            <p className="font-medium text-sm text-brand-ink">{t.sessions.folioLabel} {rx.folio}</p>
                            ...
                            {rx.medications && (
                              <p className="text-xs text-gray-600 mt-1 line-clamp-2 sm:truncate">
                                {t.sessions.medicationsLabel} {rx.medications}
                              </p>
                            )}
                          </div>
                          <button
                    ...
                    <p className="text-sm text-gray-400 italic">{t.sessions.noPrescriptions}</p>
                  )}
                </div>
              )}
        ...
        >
            <AppModalHeader>
              <div className="flex items-center justify-between gap-3">
                <div className="min-w-0">
                  <h3 className="text-lg font-semibold text-brand-ink">{t.sessions.prescriptions.newPrescription}</h3>
                  <p className="text-sm text-gray-500 truncate mt-1">
                    {t.sessions.patientPrefix} {getPatientName(selectedSession.patientId)}
                  </p>
                </div>
                <button
                  type="button"
            ...
            <AppModalBody className="space-y-5">
              <section className="rounded-lg border border-brand-border bg-gray-50 dark:bg-gray-900/50 p-4 space-y-3">
                <h4 className="text-sm font-semibold text-brand-ink">{t.sessions.patientData}</h4>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-sm">
                  <div>
                    <span className="block text-xs font-medium text-gray-500 mb-0.5">{t.common.name}</span>
                    <p className="text-brand-ink">
                      {prescriptionFormContext?.patientName ?? getPatientName(selectedSession.patientId)}
                    </p>
                  </div>
                  <div>
                    <span className="block text-xs font-medium text-gray-500 mb-0.5">{t.sessions.idOrCurp}</span>
                    <p className="text-brand-ink">
                      {prescriptionFormContext?.patientDni ?? "—"}
                    </p>
                  </div>
                  <div>
                    <span className="block text-xs font-medium text-gray-500 mb-0.5">{t.sessions.age}</span>
                    <p className="text-brand-ink">
                      {prescriptionFormContext?.patientAgeYears != null
                        ? t.sessions.yearsOld.replace("{n}", String(prescriptionFormContext.patientAgeYears))
                        : "—"}
                    </p>
                  </div>
                </div>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  ...
                  <div>
                    ...
                    <input
                      ...
                      placeholder={t.sessions.heightPlaceholder}
                      className="w-full px-4 py-2.5 bg-brand-surface text-brand-ink border border-brand-border rounded-lg focus:ring-2 focus:ring-brand-ink focus:border-transparent"
                    />
                  </div>
                </div>
              ...
              <section className="rounded-lg border border-brand-border bg-gray-50 dark:bg-gray-900/50 p-4 space-y-3">
                <h4 className="text-sm font-semibold text-brand-ink">{t.sessions.professionalData}</h4>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-sm">
                  <div>
                    <span className="block text-xs font-medium text-gray-500 mb-0.5">{t.sessions.professional}</span>
                    <p className="text-brand-ink">
                  ...
                  <div>
                    <label className="block text-sm font-medium text-brand-muted mb-1">
                      {t.sessions.professionalLicense}
                    </label>
                    <input
                      type="text"
                      ...
                      placeholder={t.sessions.licensePlaceholder}
                      className="w-full px-4 py-2.5 bg-brand-surface text-brand-ink border border-brand-border rounded-lg focus:ring-2 focus:ring-brand-ink focus:border-transparent"
                    />
                  </div>
                </div>
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">
                  {t.sessions.followUp.nextAppointment}
                </label>
                <input
                  type="date"
            ...
            <AppModalFooter>
              ...
              >
                {prescriptionSaving ? t.sessions.prescriptions.creating : t.sessions.prescriptions.create}
              </button>
              </div>
              {!prescriptionData.prescriptionText.trim() && !prescriptionData.medications.trim() && (
                <p className="text-xs text-gray-500 mt-2">
                  {t.sessions.prescriptionMinContent}
                </p>
              )}
            </AppModalFooter>
        </AppModal>
        ...
        <AppModal open onClose={closeSessionForm} maxWidth="3xl" panelId="session-form-panel">
            ...
            <form className="space-y-6" data-session-form>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  ...
                  {patientPickerMode === "loading" ? (
                    <p className={`text-sm ${formHintClass} py-2`}>{t.sessions.loadingPatients}</p>
                    ...
                    <select
                      ...
                      disabled={!!editingSession}
                    >
                      <option value="">{t.sessions.selectEllipsis}</option>
                      {sessionFormPatients
                        .filter(
                          (p) =>
                        ...
                        .map((p) => (
                          <option key={p.id} value={p.id}>
                            ...
                            {!isPatientCompleteForSessions(p) ? t.sessions.incompleteData : ""}
                          </option>
                        ))}
                    </select>
                  )}
                  {formData.patientId && (() => {
                    ...
                    <div className={`mt-2 ${semanticAlertWarningClass} space-y-2`}>
                      <p className="text-sm">
                        {t.sessions.incompletePatientWarning}
                      </p>
              ...
              <div className={`${formPanelMutedClass} space-y-3`}>
                <p className={`${formLabelClass}`}>{t.sessions.vitals.title}</p>
                <p className={`text-sm ${formHintClass}`}>
                  {t.sessions.vitalsHint}
                </p>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div>
                    <label className={`${formLabelClass} mb-1`}>{t.sessions.vitals.weightKg}</label>
                    <input
                      type="text"
                      inputMode="decimal"
                  ...
                  <div>
                    <label className={`${formLabelClass} mb-1`}>{t.sessions.vitals.heightCm}</label>
                    <input
                      type="text"
                      inputMode="decimal"
              ...
              <div className={`${formPanelMutedClass} space-y-3`}>
                <div>
                  ...
                  >
                    <option value="">{t.sessions.noTemplate}</option>
                    {sessionTemplates.map((tpl) => (
                      <option key={tpl.id} value={tpl.id}>
                        {tpl.name}
                        {tpl.isShared ? t.sessions.templateClinic : t.sessions.templatePersonal}
                      </option>
                    ))}
                  </select>
                </div>
                {sessionTemplates.length === 0 ? (
                  <p className={`text-xs ${formHintClass}`}>
                    {t.sessions.noTemplatesBefore}{" "}
                    <a href="/clinical-tools" className="underline text-brand-ink">
                      {t.sessions.clinicalToolsLink}
                    </a>
                    {t.sessions.noTemplatesAfter}
                  </p>
                ) : (
                  <p className={`text-xs ${formHintClass}`}>
                    {t.sessions.templateApplyHint}
                  </p>
                )}
                {appliedTemplateLayout ? (
                  <p className={`text-xs ${formHintClass}`}>
                    {t.sessions.templateFilteredView.replace(
                      "{n}",
                      String(countIncludedTemplateSections(appliedTemplateLayout))
                    )}
                  </p>
                ) : selectedTemplateId ? (
                  <p className={`text-xs ${formWarningClass}`}>
                    {t.sessions.templateNoSections}
                  </p>
                ) : null}
              </div>
              ...
              <div>
                <label className={`${formLabelClass} mb-1`}>
                  {resolveLayoutSectionLabel(t, sessionFormLayout, "anamnesis")}
                </label>
                <textarea
                  rows={3}
                  ...
                  placeholder={t.sessions.anamnesisPlaceholder}
                />
              </div>
              )}
              ...
              <div>
                <label className={`${formLabelClass} mb-1`}>
                  {resolveLayoutSectionLabel(t, sessionFormLayout, "physical_examination")}
                </label>
                <textarea
                  rows={3}
                  ...
                  className={formFieldResizeClass}
                  placeholder={t.sessions.examPlaceholder}
                />
              </div>
              )}
              ...
              <div>
                <label className={`${formLabelClass} mb-1`}>
                  {resolveLayoutSectionLabel(t, sessionFormLayout, "diagnosis")}
                </label>
                <textarea
                  rows={2}
              ...
              <div>
                <label className={`${formLabelClass} mb-1`}>
                  {resolveLayoutSectionLabel(t, sessionFormLayout, "treatment_plan")}
                </label>
                <textarea
                  rows={3}
              ...
              <div>
                <label className={`${formLabelClass} mb-1`}>
                  {resolveLayoutSectionLabel(t, sessionFormLayout, "clinical_notes")}
                </label>
                <textarea
                  rows={2}
              ...
              <div className="border-t border-brand-border pt-6">
                <h4 className="text-sm font-semibold text-brand-ink mb-4">{t.sessions.followUp.sectionTitle}</h4>
                ...
                <div>
                  <label className={`${formLabelClass} mb-1`}>
                    {t.sessions.followUpInstructions}
                  </label>
                  <textarea
                    rows={2}
                    ...
                    className={formFieldClassSm}
                    placeholder={t.sessions.followUpInstructionsPlaceholder}
                  />
                </div>
              </div>
              ...
              {(() => {
                ...
                const disableReason = !formData.patientId
                  ? t.sessions.selectPatientTitle
                  : !patientComplete
                    ? t.sessions.completePatientDataTitle
                    : undefined;
                  ...
                  <div className="flex flex-col gap-3 pt-4 border-t border-gray-100">
                    {showSaveBlockedHint && (
                      <p className={`text-sm ${semanticAlertWarningClass}`}>
                        {t.sessions.completePatientDataHint}
                      </p>
                    )}
                    {graceError && (
      ...
      <div className="space-y-6">
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1">
                      <h4 className="font-medium text-red-800 mb-1">
                        {t.sessions.followUp.overdueBanner} ({overdueAppointments.length})
                      </h4>
                      <div className="space-y-1.5">
                        {overdueAppointments.slice(0, 3).map(({ session, patient, daysOverdue }) => (
                          <div key={session.id} className="flex items-center justify-between text-sm">
                            <span className="text-red-700">
                              {patient?.firstName} {patient?.lastName} - {t.sessions.daysOverdue.replace("{n}", String(daysOverdue))}
                            </span>
                            <button
                              onClick={() => {
                            ...
                            >
                              {t.sessions.scheduleAppointment}
                            </button>
                          </div>
                        ))}
                      </div>
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1">
                      <h4 className="font-medium text-blue-800 mb-1">
                        {t.sessions.followUp.upcomingBanner} - {upcomingAppointments.length}
                      </h4>
                      <div className="space-y-1.5">
                        {upcomingAppointments.slice(0, 3).map(({ session, patient, daysUntil }) => (
                          <div key={session.id} className="flex items-center justify-between text-sm">
                            <span className="text-blue-700">
                              {patient?.firstName} {patient?.lastName} - {daysUntil === 0 ? t.sessions.todayRel : daysUntil === 1 ? t.sessions.tomorrowRel : t.sessions.inDays.replace("{n}", String(daysUntil))}
                            </span>
                            <div className="flex items-center gap-2">
                              <span className="text-xs text-blue-500">
                                {new Date(session.nextAppointmentDate!).toLocaleDateString(dateLocale, { weekday: "short", day: "numeric", month: "short" })}
                              </span>
                              <button
                                type="button"
                                onClick={() => handleRescheduleNextAppointment(session)}
                                className="text-xs font-medium text-blue-700 hover:text-blue-900 hover:underline"
                                title={t.sessions.rescheduleTitle}
                              >
        ...
        {clinicalListLoading && sessions.length === 0 ? (
          <ClinicalListLoading label={t.sessions.loadingSessions} />
        ) : sessions.length === 0 ? (
          <div className="bg-brand-surface rounded-xl border border-brand-border p-12 text-center">
            ...
            <p className="text-brand-muted mb-6">{t.sessions.createFirstSession}</p>
            <button
              onClick={() => setShowForm(true)}
          ...
          <div className="grid gap-4">
                  ...
                  <div className="flex-1 min-w-0 cursor-pointer" onClick={() => void openSessionDetail(session)}>
                        ...
                        if (daysUntil < 0) {
                          return (
                            <span className="px-2 py-0.5 rounded-full text-xs font-medium bg-red-100 text-red-700 flex items-center gap-1">
                              ...
                              {t.sessions.followUp.overdueChip}
                            </span>
                          );
                        } else if (daysUntil <= 7) {
                          return (
                            <span className="px-2 py-0.5 rounded-full text-xs font-medium bg-yellow-100 text-yellow-700 flex items-center gap-1">
                              ...
                              {t.sessions.followUp.upcomingChip}
                            </span>
                          );
                        }
                        return null;
                    ...
                    {session.nextAppointmentDate && (
                      <p className="text-xs text-gray-400 dark:text-gray-500 mt-1">
                        📅 {t.sessions.followUp.nextAppointment}: {new Date(session.nextAppointmentDate).toLocaleDateString(dateLocale, { day: "numeric", month: "short", year: "numeric" })}
                      </p>
                    )}
                  </div>
            ...
            {sessionsHasMore && (
              <div className="flex justify-center pt-2">
                ...
                >
                  {sessionsLoadingMore ? t.sessions.loadingMore : t.sessions.loadMoreSessions}
                </button>
              </div>
            )}
          </div>
