...
export const Sidebar = ({
  ...
  return (
    <>
      {/* Overlay for mobile - z-[60] when open so it appears above main content (z-[55]) */}
      <div
        className={`fixed inset-0 bg-black/60 backdrop-blur-sm md:hidden transition-opacity duration-300 ${
          isOpen ? "opacity-100 z-[60]" : "opacity-0 pointer-events-none z-40"
        }`}
        onClick={onClose}
      />

      {/* Sidebar - z-[60] when open on mobile so it appears above main content */}
      <aside
        className={`fixed top-0 left-0 h-full w-[85%] max-w-[300px] md:w-72 bg-[#1a1a1a] transform transition-transform duration-300 ease-out ${
          isOpen ? "translate-x-0 z-[60]" : "-translate-x-full z-50"
        } ${visibleOnDesktop ? "md:translate-x-0 md:z-50" : "md:-translate-x-full"}`}
      >
